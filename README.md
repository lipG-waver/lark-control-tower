...
- **HTML 仪表盘**（每 30 分钟自动重渲染，页面每 10 分钟自刷新）：
  🗺 战线（项目级：阶段徽章 + 进度条 + 最近里程碑）→ 🔴 P0 → 🟠 待审 → ⏰ 3 天内到期 → 📋 个人清单 → 🟢 进行中 / ⚪ 待办（按业务线折叠）
...
- 一张任务多维表格，含字段：任务标题 / 状态 / 优先级（P0🔴~P3🟢）/ 业务线 / 认领人 /
  截止日期 / 详情备注（字段名可在 config 改）
- 可选·三层结构（战线 → 项目 → 任务）：再加「层级」单选（项目/任务）、「阶段」单选
  （构想/搭建/验收/上线/运维/等外部）、「父记录」自链接字段。项目=层级为"项目"的普通记录，
  任务通过父记录挂靠；仪表盘顶部即出现「战线」区，进度条=已完成子任务/全部子任务，
  项目详情备注里 append-only 记里程碑（最后一行会显示在卡片上）。没这三个字段则该区自动隐藏
